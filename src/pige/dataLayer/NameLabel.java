...
public class NameLabel 
	...
	private Font font = 
           new Font(Constants.LABEL_FONT, Font.BOLD, Constants.LABEL_DEFAULT_FONT_SIZE);
...
//	private boolean doZoom = false;
   ...
   public NameLabel(int zoom) {
      this("");
      setFont(getFont().deriveFont(ZoomController.getZoomedValue(
              (float)Constants.LABEL_DEFAULT_FONT_SIZE, zoom)));
   }
   ...
   public void updatePosition() {
      setLocation(positionX - getPreferredSize().width,
                  positionY - Constants.NAMELABEL_OFFSET);
   }
   ...
   public void zoomUpdate(int value) {
      setFont(getFont().deriveFont(ZoomController.getZoomedValue(
              (float)Constants.LABEL_DEFAULT_FONT_SIZE, value)));
      updateSize();
   }
