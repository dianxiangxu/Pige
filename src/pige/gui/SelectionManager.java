...
public class SelectionManager 
   ...
   private void processSelection(MouseEvent e) {
      ...
      ArrayList <GraphObject> pnObjects = view.getGraphObjects();
      for (GraphObject pnObject : pnObjects) {
   ...
   public void clearSelection() {
      // Get all the objects in the current window
      ArrayList <GraphObject> graphObjects = view.getGraphObjects();
      for (GraphObject graphObject : graphObjects) {    
         if (graphObject.isSelectable()) {
            graphObject.deselect();
         }
      }
   }
   ...
   public int getSelectionCount() {
      ...
      for (int i=0; i< netObj.length; i++) {
         ...
         if ((netObj[i] instanceof GraphArc) && ((GraphObject)netObj[i]).isSelectable())	{
            GraphArc thisArc = (GraphArc)netObj[i];
            GraphArcPath thisArcPath = thisArc.getArcPath();
         ...
         if ((netObj[i] instanceof GraphAbstractNode) && 
                 ((GraphObject)netObj[i]).isSelectable()) {
            if (((GraphAbstractNode)netObj[i]).isSelected()) {
               selectionCount++;
            }
         }
      }
